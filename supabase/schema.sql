...
update public.memberships
set role = 'sales_rep'
where role not in ('sales_rep', 'store_keeper', 'production_manager', 'admin', 'ceo');
...
check (role in ('sales_rep', 'store_keeper', 'production_manager', 'admin', 'ceo'));
...
begin
  ...
  insert into public.workspace_message_deletions (
    client_id,
    message_id,
    membership_id
  )
...
revoke all on function public.delete_my_workspace_messages(uuid, uuid[], boolean) from public, anon;
revoke all on function public.clear_my_workspace_conversation(uuid, uuid, text) from public, anon;
...
revoke all privileges on table
  ...
  public.workspace_message_deletions,
...
-- CEO-only record-level deletion used by finance, activity, and sales orders.
...
begin
  ...
  if v_role <> 'ceo' then raise exception 'Only the CEO can permanently delete company records'; end if;
  if coalesce(array_length(v_ids, 1), 0) = 0 then raise exception 'Choose at least one record to delete'; end if;
  ...
  if v_collection is null then raise exception 'Unsupported deletion scope'; end if;
  ...
  if v_scope = 'activity' then
    delete from public.activity_logs where client_id = p_client_id and id::text = any(v_ids);
    get diagnostics v_count = row_count;
    v_deleted := v_deleted + v_count;
  elsif v_scope in ('representative_credit_history', 'customer_credit_history') then
    delete from public.credit_limit_history where client_id = p_client_id and id::text = any(v_ids);
    get diagnostics v_count = row_count;
    v_deleted := v_deleted + v_count;
  elsif v_scope in ('representative_credit_limits', 'customer_credit_limits') then
    delete from public.credit_limits where client_id = p_client_id and id::text = any(v_ids);
    get diagnostics v_count = row_count;
    v_deleted := v_deleted + v_count;
  end if;
  ...
  delete from public.workspace_operation_events where client_id = p_client_id;

  return jsonb_build_object('scope', v_scope, 'deleted', v_deleted, 'completedAt', now());
end;
$$;
