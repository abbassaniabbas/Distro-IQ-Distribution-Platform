...
const tenantRoles = new Set(["sales_rep", "store_keeper", "accountant", "ceo"]);
...
async function updateUser(adminClient: ReturnType<typeof createClient>, caller: { id: string; email?: string }, payload: Record<string, unknown>) {
  ...
  if (actionType === "reset-password") {
    const redirectTo = cleanText(payload.redirectTo) || `${new URL(cleanText(payload.origin) || "http://127.0.0.1:8080").origin}/#/reset-password`;
    await adminClient.auth.resetPasswordForEmail(membership.email, { redirectTo });
